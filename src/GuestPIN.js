...
const headCells = [
  ...
  { id: 'pintype', label: 'PIN Type' },
  { id: 'pinusetime', label: 'PIN Use Time' },
  { id: 'dateexpiry', label: 'PIN Expiry Date' },
...
/*useEffect(() => {  console.log('use effect clicked'); if(resetClick){
  ...
  const ShowLog = () =>{
    return(<Grid container spacing={2}> 
      ...
      <Typography sx={mainThemeStyle.popUpLable}> {EnLog[currentId].guestpin} </Typography>
        ...
        fn: items => {
            if (filterText == "")
              ...
              if(shownameType){
                ...
                return items.filter(x => x.guestpin.toString().toLowerCase().includes(filterText.toLowerCase()))
                    ...
                    <TableBody>
                        {
                            recordsAfterPagingAndSorting().map(item =>
                                ...
                                >
                                  ...
                                  <TableCell align="left" sx={mainThemeStyle.cellRow}>{item.guestpin}</TableCell>
