...
const headCells = [
  { id: 'preopertyname', label: 'Contact Name' },
  { id: 'comapnyname', label: 'Company Name'},
  {id: 'distributorcomapnyname', label: 'Distributor Company Name'},
...
/*useEffect(() => {  console.log('use effect clicked'); if(resetClick){
  ...
  const ChangeId = (val,val2) =>{
    ...
    if(val2 == 0){
      navigate('../dealer/editdealer', { replace: true })
    }else if(val2 == 2){
      setCurrentId(val);
      let deletid;
        ...
        fn: items => {
            if (filterText == "")
              ...
              if(shownameType){
                ...
                return items.filter(x => x.distributorcomapnyname.toString().toLowerCase().includes(filterText.toLowerCase()))
                    ...
                    <TableBody>
                        {
                            recordsAfterPagingAndSorting().map(item =>
                                ...
                                >
                                  ...
                                  <TableCell align="left" sx={mainThemeStyle.cellRow}>{item.distributorcomapnyname}</TableCell>
